...
import com.kct.tplusmcscenter.view.widget.SizeThreeTwoAppWidget
...
class UserDataThreeTwoIntentService : JobIntentService() {
    companion object {
        ...
        private const val JOB_ID = 4007
        const val ACTION_UPDATE_DATA = "com.test.ktc.widget.action.UPDATE_DATA_THREE_TWO"
        const val ACTION_UPDATE_ERROR = "com.test.ktc.widget.action.UPDATE_ERROR_THREE_TWO"

        internal fun enqueueWork(context: Context?, intent: Intent) {
            enqueueWork(context!!, UserDataThreeTwoIntentService::class.java, JOB_ID, intent)
        }
    }
    ...
    private fun defaultValueBroadcast() {
        sendBroadcast(
            Intent(ACTION_UPDATE_DATA).apply {
                ...
                putExtra("call", "-분")
                putExtra("sms", "-건")
                putExtra("smspercent", "100")
                putExtra("voicepercent", "100")
                putExtra("datapercent", "100")
            }
        )
    }
