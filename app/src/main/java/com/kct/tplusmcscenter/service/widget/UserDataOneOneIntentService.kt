...
@AndroidEntryPoint
class UserDataOneOneIntentService : JobIntentService() {
    companion object {
        private const val TAG = "[UserDataService]"
        private const val JOB_ID = 4005
        const val ACTION_UPDATE_DATA = "com.test.ktc.widget.action.UPDATE_DATA_ONE_ONE"
        const val ACTION_UPDATE_ERROR = "com.test.ktc.widget.action.UPDATE_ERROR_ONE_ONE"

        internal fun enqueueWork(context: Context?, intent: Intent) {
            enqueueWork(context!!, UserDataOneOneIntentService::class.java, JOB_ID, intent)
        }
    }
